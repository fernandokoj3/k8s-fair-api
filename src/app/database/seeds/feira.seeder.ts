...
export class CreateFeiras implements Seeder {
  public async run(
    ...
    let file = resolve(__dirname, './feira-livre.csv');
    ...
    await repository
      ...
      .into(FeiraLivre)
      .values(rows)
      .orIgnore('("longitude", "latitude") DO NOTHING')
      .execute();
  }
  ...
  private async process(file: string) {
    return new Promise<FeiraLivre[]>((resolve, reject) => {
      ...
      createReadStream(file)
        ...
        .on('data', async row => {
          let feiraRequest = plainToInstance(
            FeiraLivreImport,
            Object.assign({}, row),
          );
          let ent: FeiraLivre = Object.assign(
            {},
            feiraRequest as any as FeiraLivre,
          );
